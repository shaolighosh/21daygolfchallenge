...
class Individual_service_management extends CI_Controller {
	...
	public function index()
	{
		...
		if($this->session->userdata('admin_logged_in')){
	        ...
	        $result["links"] = $this->pagination->create_links();
	        	...
	        	$result['services'] =$this->Common_model->getDatawithlimit($this->table_individual_services,array(),'id',$page,10);
			...
			$data['content'] = $this->load->view('admin/individual-service-management/list',$result, true);
	...
	public function add()
	{
		if($this->session->userdata('admin_logged_in')){
			...
			if($this->form_validation->run() == false) {
				$allData = '';
				$data['content'] = $this->load->view('admin/individual-service-management/add',$allData, true);
				$this->load->view('template/admin/main',$data);
			} else {
				$date = date('Y-m-d h:m:s');
				$userData = array('service_name'=>$this->input->post('service_name'),'square_foot'=>$this->input->post('square_foot'),'total_service'=>1,'service_prefix'=>$this->input->post('service_prefix'),'service_min_size'=>$this->input->post('service_min_size'),'service_min_time'=>$this->input->post('service_min_time'),'service_min_price'=>$this->input->post('service_min_price'),'service_incremental'=>$this->input->post('service_incremental'),'description'=>$this->input->post('description'),'created_at'=> $date);
				 $insert_id = $this->Common_model->insertData($this->table_individual_services,$userData);
				 $this->session->set_flashdata('success', 'Individual Service Added Successfully.');
	...
	public function edit()
	{
		if($this->session->userdata('admin_logged_in')){
			if($this->input->post('service_name')){
				$serviceData = array('service_name'=>$this->input->post('service_name'),'service_prefix'=>$this->input->post('service_prefix'),'service_min_size'=>$this->input->post('service_min_size'),'service_min_time'=>$this->input->post('service_min_time'),'service_min_price'=>$this->input->post('service_min_price'),'service_incremental'=>$this->input->post('service_incremental'),'description'=>$this->input->post('description'));
				$update = $this->Common_model->dataUpdate($this->table_individual_services,array('id'=> $this->input->post('serviceId') ),$serviceData);
				
				 $this->session->set_flashdata('success', 'Individual Service Updated Successfully.');
				 redirect('admin/individual-service-management/editservicedescription/'.$this->input->post('serviceId'));
				...
				$data['content'] = $this->load->view('admin/individual-service-management/edit',$result, true);
				$this->load->view('template/admin/main',$data);
			}
		}else{
			$this->load->view('admin/login');
	...
	public function delete($id)
	{
		...
		if($this->session->userdata('admin_logged_in')){
			...
			$update = $this->Common_model->delete_data($this->table_individual_services,array('id'=> $id ));
			
			$this->session->set_flashdata('success', 'Individual Service Delete Successfully.');
			redirect('admin/individual-service-management/');
		}else{
			redirect('admin/login');
		}
