		 ...
		 $video_id = ($this->uri->segment(4))?$this->uri->segment(4):$this->input->post('id');
			if($this->session->userdata('admin_logged_in')){
				...
				//	$userId = $this->session->userdata('admin_logged_in');
							...
							$this->Common_model->update_data('golfersu_video_resource',array('id' => $this->input->post('resourceId')),$updateData);
					...
					$allData = array();
					$allData['steps'] = $this->Common_model->getData($this->table_golfersu_steps,array(),'id');
					$allData['videoDetails'] = $this->Common_model->getSingle('golfersu_video_resource',array('id'=> $video_id ));
