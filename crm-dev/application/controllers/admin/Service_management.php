...
class Service_management extends CI_Controller {
	...
	public function index()
	{
		...
		if($this->session->userdata('admin_logged_in')){
			...
			$config = array();
	        $config["base_url"] = base_url() . "admin/service-management";
			...
			$data['content'] = $this->load->view('admin/service-management/list',$result, true);
	...
	public function add()
	{
		if($this->session->userdata('admin_logged_in')){
			...
			if($this->form_validation->run() == false) {
				...
				$allData['services'] = $this->Common_model->getAllData($this->table_core_services, array());
				$data['content'] = $this->load->view('admin/service-management/add',$allData, true);
				
				$this->load->view('template/admin/main',$data);
			} else {
				$date = date('Y-m-d h:m:s');
			$userData = array('service_type'=>$this->input->post('service_type'),'service_name'=>$this->input->post('service_name'),'program_service'=>$this->input->post('program_service'),'square_foot'=>$this->input->post('square_foot'),'total_service'=>$this->input->post('total_service'),'service_prefix'=>$this->input->post('service_prefix'),'service_min_size'=>$this->input->post('service_min_size'),'service_min_time'=>$this->input->post('service_min_time'),'service_min_price'=>$this->input->post('service_min_price'),'service_incremental'=>$this->input->post('service_incremental'),'related_service_id'=>$this->input->post('related_service_id'),'with_service_price'=>$this->input->post('with_service_price'),'service_incremental_price'=>$this->input->post('service_incremental_price'),'short_description'=>$this->input->post('short_description'),'description'=>$this->input->post('description'),'created_at'=> $date);
				...
				redirect('admin/service-management/addservicedescription/'.$insert_id);
			}	
		}else{
			redirect('admin/login');
		}
	...
	public function addservicedescription()
	{
		...
		if($this->session->userdata('admin_logged_in')){
			...
			$total_service = $this->input->post('roundservice');
				...
				$userData = array('service_type'=>$this->input->post('service_type'),'service_id'=>$this->input->post('service_id'),'round_content'=>$round_content,
				...
				$this->Common_model->insertData($this->table_service_round_description,$userData);
				 ...
				 redirect('admin/service-management/');
				 
			}else{
				$id = $this->uri->segment(4);
				$result['service'] = $this->Common_model->getSingle($this->table_core_services, array('id'=> $id));
				$data['content'] = $this->load->view('admin/service-management/addservicedescription',$result, true);
				$this->load->view('template/admin/main',$data);
			}
		}else{
	...
	public function edit()
	{
		if($this->session->userdata('admin_logged_in')){
			if($this->input->post('service_name')){
				$serviceData = array('service_type'=>$this->input->post('service_type'),'service_name'=>$this->input->post('service_name'),'program_service'=>$this->input->post('program_service'),'service_prefix'=>$this->input->post('service_prefix'),'service_min_size'=>$this->input->post('service_min_size'),'service_min_time'=>$this->input->post('service_min_time'),'service_min_price'=>$this->input->post('service_min_price'),'service_incremental'=>$this->input->post('service_incremental'),'related_service_id'=>$this->input->post('related_service_id'),'with_service_price'=>$this->input->post('with_service_price'),'service_incremental_price'=>$this->input->post('service_incremental_price'),'short_description'=>$this->input->post('short_description'),'description'=>$this->input->post('description'));
				$update = $this->Common_model->dataUpdate($this->table_core_services,array('id'=> $this->input->post('serviceId') ),$serviceData);
				
				 $this->session->set_flashdata('success', 'Service Updated Successfully.');
				 redirect('admin/service-management/editservicedescription/'.$this->input->post('serviceId'));
				 
			}else{
				$id = $this->uri->segment(4);
				$result['service'] = $this->Common_model->getSingle($this->table_core_services, array('id'=> $id));
				$result['allservice'] = $this->Common_model->getAllData($this->table_core_services, array());
				
				$data['content'] = $this->load->view('admin/service-management/edit',$result, true);
				$this->load->view('template/admin/main',$data);
			}
		}else{
			$this->load->view('admin/login');
	...
	public function editservicedescription()
	{
		...
		if($this->session->userdata('admin_logged_in')){
			...
			$result['rounds'] = $this->Common_model->getAllData($this->table_service_round_description, array('service_id'=> $id, 'service_type'=> $service_type));
				$data['content'] = $this->load->view('admin/service-management/editservicedescription',$result, true);
				$this->load->view('template/admin/main',$data);
			}
		}else{
			$this->load->view('admin/login');
	...
	public function view()
	{
		if($this->session->userdata('admin_logged_in')){
				...
				$data['content'] = $this->load->view('admin/service-management/view',$result, true);
				$this->load->view('template/admin/main',$data);
