...
class Pest_controls_service_management extends CI_Controller {
	...
	public function __construct()  
    {
		...
		$this->table_outdoor_pest_controls = 'outdoor_pest_controls';
	...
	public function index()
	{
		...
		if($this->session->userdata('admin_logged_in')){
			if(isset($_GET['id'])){
				$resultCount = $this->Common_model->numrowsDbQuery("Select id from $this->table_outdoor_pest_controls WHERE id='".$_GET['id']."' ");
				}elseif (isset($_GET['date'])) {
					$resultCount = $this->Common_model->numrowsDbQuery("Select id from $this->table_outdoor_pest_controls WHERE create_date='".$_GET['date']."' ");
				
			}else{
				$resultCount = $this->Common_model->numrows($this->table_outdoor_pest_controls,array());
			}
			$config = array();
	        $config["base_url"] = base_url() . "admin/pest-controls-service-management";
	        ...
	        $result['page'] = $page;
	        $result['Allcourse'] = $this->Common_model->getData($this->table_outdoor_pest_controls,array(),'id');  
	        $result["links"] = $this->pagination->create_links();
	        
	        	$result['services'] =$this->Common_model->getDatawithlimit($this->table_outdoor_pest_controls,array(),'id',$page,10);
			...
			$data['content'] = $this->load->view('admin/pest-controls-service-management/list',$result, true);
	...
	public function add()
	{
		if($this->session->userdata('admin_logged_in')){
			...
			if($this->form_validation->run() == false) {
				$allData = '';
				$data['content'] = $this->load->view('admin/pest-controls-service-management/add',$allData, true);
				$this->load->view('template/admin/main',$data);
			} else {
				...
				$userData = array('service_name'=>$this->input->post('service_name'),'square_foot'=>$this->input->post('square_foot'),'total_service'=>$this->input->post('total_service'),'service_prefix'=>$this->input->post('service_prefix'),'service_min_size'=>$this->input->post('service_min_size'),'service_min_time'=>$this->input->post('service_min_time'),'service_min_price'=>$this->input->post('service_min_price'),'service_incremental'=>$this->input->post('service_incremental'),'description'=>$this->input->post('description'),'created_at'=> $date);
				 $insert_id = $this->Common_model->insertData($this->table_outdoor_pest_controls,$userData);
				 $this->session->set_flashdata('success', 'Pest Control Service Added Successfully.');
				redirect('admin/pest-controls-service-management/addservicedescription/'.$insert_id);
			}	
		}else{
			redirect('admin/login');
		}
	...
	public function addservicedescription()
	{
		...
		if($this->session->userdata('admin_logged_in')){
			if($this->input->post('roundservice')){
				...
				$userData = array('service_type'=>2,'service_id'=>$this->input->post('service_id'),'round_content'=>$round_content,
				...
				$this->Common_model->insertData($this->table_service_round_description,$userData);
				 ...
				 $this->session->set_flashdata('success', 'Pest Control Service Updated Successfully.');
				 redirect('admin/pest-controls-service-management/');
				 
			}else{
				$id = $this->uri->segment(4);
				$result['service'] = $this->Common_model->getSingle($this->table_outdoor_pest_controls, array('id'=> $id));
				$data['content'] = $this->load->view('admin/pest-controls-service-management/addservicedescription',$result, true);
				$this->load->view('template/admin/main',$data);
			}
		}else{
	...
	public function edit()
	{
		if($this->session->userdata('admin_logged_in')){
			if($this->input->post('service_name')){
				...
				$update = $this->Common_model->dataUpdate($this->table_outdoor_pest_controls,array('id'=> $this->input->post('serviceId') ),$serviceData);
				
				 $this->session->set_flashdata('success', 'Pest Control Service Updated Successfully.');
				 redirect('admin/pest-controls-service-management/editservicedescription/'.$this->input->post('serviceId'));
				...
				$data['content'] = $this->load->view('admin/pest-controls-service-management/edit',$result, true);
				$this->load->view('template/admin/main',$data);
			}
		}else{
			$this->load->view('admin/login');
	...
	public function editservicedescription()
	{
		...
		if($this->session->userdata('admin_logged_in')){
			if($this->input->post('roundservice')){
				...
				$result['service'] = $this->Common_model->getSingle($this->table_outdoor_pest_controls, array('id'=> $id));
				$data['content'] = $this->load->view('admin/pest-controls-service-management/editservicedescription',$result, true);
				$this->load->view('template/admin/main',$data);
			}
		}else{
			$this->load->view('admin/login');
	...
	public function view()
	{
		if($this->session->userdata('admin_logged_in')){
				...
				$data['content'] = $this->load->view('admin/pest-controls-service-management/view',$result, true);
				$this->load->view('template/admin/main',$data);
	...
	public function delete($id)
	{
		...
		if($this->session->userdata('admin_logged_in')){
			...
			$update = $this->Common_model->delete_data($this->table_outdoor_pest_controls,array('id'=> $id ));
			
			$this->session->set_flashdata('success', 'Pest Control Service Delete Successfully.');
			redirect('admin/pest-controls-service-management/');
		}else{
			redirect('admin/login');
		}
