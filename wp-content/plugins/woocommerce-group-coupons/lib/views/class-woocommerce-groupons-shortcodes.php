...
class WooCommerce_Groupons_Shortcodes {
	...
	public static function check_validity( &$coupon, &$valid, &$error_codes ) {
		...
		if ( $coupon->get_id() ) {
			...
			if ( $coupon->get_discount_type() != 'fixed_product' && $coupon->get_discount_type() != 'percent_product' ) {
	...
	public static function get_discount_info( $coupon, $atts = array() ) {
		...
		switch ( $coupon->get_discount_type() ) {
			...
			case 'percent_product' :
				if ( count( $coupon->get_product_ids() ) > 0 ) {
					if ( count( $products ) > 0 ) {
						$result = sprintf( __( '%s%s Discount on %s', 'woocommerce-group-coupons' ), $coupon->get_amount(), $amount_suffix, implode( $product_delimiter, $products ) );
					} else {
						$result = sprintf( __( '%s%s Discount on selected products', 'woocommerce-group-coupons' ), $coupon->get_amount(), $amount_suffix );
					}
				} else if ( count( $coupon->get_product_categories() ) > 0 ) {
					$result = sprintf( __( '%s%s Discount in %s', 'woocommerce-group-coupons' ), $coupon->get_amount(), $amount_suffix, implode( $category_delimiter, $categories ) );
				} else if ( count( $coupon->get_excluded_product_ids() ) > 0 || count( $coupon->get_excluded_product_categories() ) > 0 ) {
					$result = sprintf( __( '%s%s Discount on selected products', 'woocommerce-group-coupons' ), $coupon->get_amount(), $amount_suffix );
				} else {
					$result = sprintf( __( '%s%s Discount', 'woocommerce-group-coupons' ), $coupon->get_amount(), $amount_suffix );
				}
				break;
			...
			case 'recurring_fee' :
			case 'recurring_percent' :
				$discount_name = __( 'Subscription Discount', 'woocommerce-group-coupons' );
				if ( $coupon->get_discount_type() == 'sign_up_fee' || $coupon->get_discount_type() == 'sign_up_fee_percent' ) {
					$discount_name = __( 'Sign Up Discount', 'woocommerce-group-coupons' );
				}
				if ( count( $coupon->get_product_ids() ) > 0 ) {
					if ( count( $products ) > 0 ) {
						$result = sprintf( __( '%s%s %s on %s', 'woocommerce-group-coupons' ), $coupon->get_amount(), $amount_suffix, $discount_name, implode( $product_delimiter, $products ) );
					} else {
						$result = sprintf( __( '%s%s %s on selected products', 'woocommerce-group-coupons' ), $coupon->get_amount(), $amount_suffix, $discount_name );
					}
				} else if ( count( $coupon->get_product_categories() ) > 0 ) {
					$result = sprintf( __( '%s%s %s in %s', 'woocommerce-group-coupons' ), $coupon->get_amount(), $amount_suffix, $discount_name, implode( $category_delimiter, $categories ) );
				} else if ( count( $coupon->get_excluded_product_ids() ) > 0 || count( $coupon->get_excluded_product_categories() ) > 0 ) {
					$result = sprintf( __( '%s%s %s on selected products', 'woocommerce-group-coupons' ), $coupon->get_amount(), $amount_suffix, $discount_name );
				} else {
					$result = sprintf( __( '%s%s %s', 'woocommerce-group-coupons' ), $coupon->get_amount(), $amount_suffix, $discount_name );
				}
				break;
		}
