...
// 		add_action( 'admin_print_styles-' . $admin_page, array( __CLASS__, 'admin_print_styles' ) );
	...
	public static function woocommerce_groupons() {
		...
		if ( !current_user_can( 'manage_woocommerce' ) ) {
			wp_die( __( 'Access denied.', 'woocommerce-group-coupons' ) );
		}
		...
		echo '<h2>' . __( 'Group Coupons', 'woocommerce-group-coupons' ) . '</h2>';
		...
		if ( WOO_GROUPONS_GROUPS_IS_ACTIVE ) {
			...
			echo '<p>';
			echo __( '<h3>Groups</h3>', 'woocommerce-group-coupons' );
			echo '</p>';

			echo '<h4>' . __( 'Automatic group coupons', 'woocommerce-group-coupons' ) . '</h4>';

			echo '<p>';
			echo '<label>';
			printf( '<input name="%s" type="checkbox" %s />', WOO_GROUPONS_AUTO_COUPONS, $auto_coupons ? ' checked="checked" ' : '' );
			echo ' ';
			_e( 'Enable automatic coupons', 'woocommerce-group-coupons' );
			echo '</label>';
			echo '</p>';
			echo '<p class="description">';
			_e( 'Automatic group coupons are coupons that are applied to group members automatically.', 'woocommerce-group-coupons' );
			echo '</p>';

			echo '<h4>' . __( 'Coupon error message', 'woocommerce-group-coupons' ) . '</h4>';

			echo '<p>';
			echo '<label>';
			printf( '<input name="%s" type="checkbox" %s />', WOO_GROUPONS_SHOW_YOU_MUST_BE_A_MEMBER, $show_you_must_be_a_member ? ' checked="checked" ' : '' );
			echo ' ';
			_e( 'Show the customized message', 'woocommerce-group-coupons' );
			echo '</label>';
			echo '</p>';
			echo '<p class="description">';
			_e( 'Show a customized error message when applying a coupon fails due to the user not being a member of a required group.', 'woocommerce-group-coupons' );
			echo '</p>';
			...
			echo '</p>';
			echo '<p class="description">';
			_e( 'Use the <code>%s</code> placeholder where the names of the groups should be shown.', 'woocommerce-group-coupons' );
			echo ' ';
			printf( __( 'The default error message is <blockquote>%s</blockquote>', 'woocommerce-group-coupons' ), htmlentities( WOO_GROUPONS_YOU_MUST_BE_A_MEMBER_DEFAULT ) );
			echo '</label>';
			echo '</p>';
		} else {
			...
			echo '</p>';
			echo '<p class="description">';
			echo __( 'Related settings will be shown if <em>Groups</em> is activated.', 'woocommerce-group-coupons' );
			echo '</p>';
			echo '</div>';
		}

		echo '<h3>' . __( 'Roles', 'woocommerce-group-coupons' ) . '</h3>';

		echo '<p>';
		echo '<label>';
		printf( '<input name="%s" type="checkbox" %s />', WOO_GROUPONS_ENABLE_ROLES, $enable_roles ? ' checked="checked" ' : '' );
		echo ' ';
		_e( 'Enable coupon restrictions based on roles.', 'woocommerce-group-coupons' );
		echo '</label>';
		echo '</p>';
		echo '<p class="description">';
		_e( 'Coupons can be limited by user role when this option is enabled.', 'woocommerce-group-coupons' );
		echo '</p>';

		echo '<h3>' . __( 'Logout', 'woocommerce-group-coupons' ) . '</h3>';

		echo '<p>';
		echo '<label>';
		printf( '<input name="%s" type="checkbox" %s />', WOO_GROUPONS_LOGOUT_REMOVE_COUPONS, $logout_remove_coupons ? ' checked="checked" ' : '' );
		echo ' ';
		_e( 'Remove coupons after logout.', 'woocommerce-group-coupons' );
		echo '</label>';
		echo '</p>';
		echo '<p class="description">';
		_e( 'When this option is enabled, all coupons are removed from the cart after a user logs out.', 'woocommerce-group-coupons' );
		echo '</p>';

		echo '<p>';
		echo wp_nonce_field( 'set', self::NONCE, true, false );
		echo '<input class="button" type="submit" name="submit" value="' . __( 'Save', 'woocommerce-group-coupons' ) . '"/>';
		echo '</p>';
		echo '</div>';
