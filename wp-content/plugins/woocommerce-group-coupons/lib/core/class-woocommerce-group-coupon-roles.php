...
class WooCommerce_Group_Coupon_Roles {
	...
	public static function woocommerce_coupon_data_panels() {
		...
		echo '</div>'; // .options_group
				...
				echo '<div class="options_group">';
		echo '<p>';
		echo __( '<strong>Exclude Roles</strong> - exclude roles from using this coupon', 'woocommerce-group-coupons' );
		echo '</p>';
		...
		if ( count( $roles ) > 0 ) {
			echo '<p class="form-field">';
			if ( !defined( 'WC_VERSION' ) || ( version_compare( WC_VERSION , '2.3.0' ) < 0 ) ) {
				printf( '<select class="_groupon_exclude_roles multiselect" name="_groupon_exclude_roles[]" multiple="multiple" title="%s">', esc_attr( __( 'No restriction', 'woocommerce-group-coupons' ) ) );
			} else {
				echo '<label for="_groupon_exclude_roles">';
				_e( 'Exclude Roles', 'woocommerce-group-coupons' );
				echo '</label>';
				echo '<select id="_groupon_exclude_roles" name="_groupon_exclude_roles[]" style="width: 50%;"  class="wc-enhanced-select" multiple="multiple" data-placeholder="' . __( 'No roles', 'woocommerce-group-coupons' ) . '">';
			}

			foreach( $roles as $role => $name ) {
				printf( '<option value="%s" %s>%s</option>', esc_attr( $role ), in_array( $role, $coupon_exclude_roles ) ? ' selected="selected" ' : '', wp_filter_nohtml_kses( $name ) );
			}
			echo '</select>';
			echo '</p>';
			if ( !defined( 'WC_VERSION' ) || ( version_compare( WC_VERSION , '2.3.0' ) < 0 ) ) {
				echo '<script type="text/javascript">';
				echo 'if (typeof jQuery !== "undefined") {';
				echo 'jQuery("select._groupon_exclude_roles").chosen();';
				echo '}';
				echo '</script>';
			}
		} else {
		...
		echo '<p>';
		echo __( 'If any role is selected, the coupon cannot be used by users with those roles.', 'woocommerce-group-coupons' );
		echo '</p>';
		echo '</div>'; // .options_group
