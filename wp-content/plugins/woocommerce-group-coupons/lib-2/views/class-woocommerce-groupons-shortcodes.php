	...
	public static function get_discount_info( $coupon, $atts = array() ) {
		...
		switch ( $coupon->type ) {
			...
			case 'percent_product' :
				if ( sizeof( $coupon->product_ids ) > 0 ) {
					if ( count( $products ) > 0 ) {
						$result = sprintf( __( '%s%s Discount on %s', 'woocommerce-group-coupons' ), $coupon->amount, $amount_suffix, implode( $product_delimiter, $products ) );
					} else {
						$result = sprintf( __( '%s%s Discount on selected products', 'woocommerce-group-coupons' ), $coupon->amount, $amount_suffix );
					}
				} else if ( sizeof( $coupon->product_categories ) > 0 ) {
					$result = sprintf( __( '%s%s Discount in %s', 'woocommerce-group-coupons' ), $coupon->amount, $amount_suffix, implode( $category_delimiter, $categories ) );
				} else if ( sizeof( $coupon->exclude_product_ids ) > 0 || sizeof( $coupon->exclude_product_categories ) > 0 ) {
					$result = sprintf( __( '%s%s Discount on selected products', 'woocommerce-group-coupons' ), $coupon->amount, $amount_suffix );
				} else {
					$result = sprintf( __( '%s%s Discount', 'woocommerce-group-coupons' ), $coupon->amount, $amount_suffix );
				}
				break;
			...
			case 'recurring_percent' :
				...
				if ( sizeof( $coupon->product_ids ) > 0 ) {
					if ( count( $products ) > 0 ) {
						$result = sprintf( __( '%s%s %s on %s', 'woocommerce-group-coupons' ), $coupon->amount, $amount_suffix, $discount_name, implode( $product_delimiter, $products ) );
					} else {
						$result = sprintf( __( '%s%s %s on selected products', 'woocommerce-group-coupons' ), $coupon->amount, $amount_suffix, $discount_name );
					}
				} else if ( sizeof( $coupon->product_categories ) > 0 ) {
					$result = sprintf( __( '%s%s %s in %s', 'woocommerce-group-coupons' ), $coupon->amount, $amount_suffix, $discount_name, implode( $category_delimiter, $categories ) );
				} else if ( sizeof( $coupon->exclude_product_ids ) > 0 || sizeof( $coupon->exclude_product_categories ) > 0 ) {
					$result = sprintf( __( '%s%s %s on selected products', 'woocommerce-group-coupons' ), $coupon->amount, $amount_suffix, $discount_name );
				} else {
					$result = sprintf( __( '%s%s %s', 'woocommerce-group-coupons' ), $coupon->amount, $amount_suffix, $discount_name );
				}
				break;
		}
